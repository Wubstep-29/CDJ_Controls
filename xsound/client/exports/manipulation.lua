...
function setPitch(name_, rate_)
    ...
    if soundInfo[name_] ~= nil then soundInfo[name_].pitch = rate_ end
end

exports('setPitch', setPitch)

function setEQ(name_, band_, gainDb_)
    SendNUIMessage({
        status = "eq",
        name = name_,
        band = band_,
    ...
    if soundInfo[name_] ~= nil then
        soundInfo[name_].eq = soundInfo[name_].eq or { hi = 0, mid = 0, lo = 0 }
        soundInfo[name_].eq[band_] = gainDb_
    end
end
